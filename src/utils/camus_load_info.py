...
def camus_generate_folds(nfolds=10, random_state=None, drop_list=None):
    '''
    ...
    7/20: added drop_list to do this whole fold generation on only the subset of 
    ehr not in the drop_list of indexes.
    '''
    ...
    for k, (train_pats, test_pats) in enumerate(kf):
        val_pats = kf[(k+1)%nfolds][1] # The validation for this fold is the test of the next fold.
